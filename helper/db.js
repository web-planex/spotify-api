class DbUtil {
  ...
  async insertTrack(input) {
    ...
    let succeeded = true;
    const obj = await this.trackModel
      .create(input, {
        transaction,
      })
      .catch((e) => {
        ...
        if (/SequelizeUniqueConstraintError/.test(e.stack)) {
          console.warn(
            `Insert ignored because of already existing record for ISRC ${input.ISRC}`
          );
        } else {
    ...
    if (succeeded) {
      transaction.commit();
      console.log(
        `testTrackInsert() returned: ${JSON.stringify(obj)}, id = ${obj.id}`
      );
    }
  ...
  async insertArtistArray(track_id, artists) {
    ...
    const transaction = await this.sequelize.transaction();
    const obj = await this.artistModel
      .bulkCreate(artistsInput, {
        transaction,
      })
      .catch((e) => {
        console.log(`Error = ${e}, stacktrace = ${e.stack}`);
