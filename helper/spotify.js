...
const getToken = async () => {
  const { SPOTIFY_BASE_AUTH_URL, SPOTIFY_TOKEN_ENDPOINT, SPOTIFY_AUTH } =
    process.env;
  ...
  await fetch(url, {
    ...
    .catch((e) => {
      console.log(
        `Error fetching spotify token = ${e}, stack trace =${e.stack}`
      );
    });
  return token;
};
...
const getTrackByISRC = async (ISRC) => {
  ...
  const url = `${SPOTIFY_BASE_URL}${SPOTIFY_SEARCH_ENDPOINT}?${querystring.stringify(
    params
  )}`;
  let result;
  await fetch(url, {
...
const getMeta = (spotifyData, idx) => {
  const mostPopularItem = spotifyData.tracks.items.sort(
    (item1, item2) => item2.popularity - item1.popularity
  )[spotifyData.tracks.items.length - 1];
